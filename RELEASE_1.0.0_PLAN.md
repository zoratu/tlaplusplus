...
- [x] **T2. Compiled-vs-interpreted proptest equivalence.** Done — `tests/compiled_vs_interpreted.rs` (612 lines) generates well-typed Int/Bool/SetInt/SeqInt/RecAB/Str expressions over a fixed state, asserts `eval_expr` and `eval_compiled(compile_expr(...))` agree on every case (or both error). Wired into `.github/workflows/diff-tlc.yml` at `PROPTEST_CASES=256`. T2.1 + T2.2 closed (single shared root cause + one masked arithmetic bug); **T2.3 remains** — the harness now fails-fast only on the documented `..`/`\subseteq` precedence shape. 14 tests in the file (10 sanity + 1 proptest, plus 3 T2.1/T2.2 regression tests in `src/tla/compiled_expr.rs::tests`).
