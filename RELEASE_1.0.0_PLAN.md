...
- [x] **T2. Compiled-vs-interpreted proptest equivalence.** Done — `tests/compiled_vs_interpreted.rs` (612 lines) generates well-typed Int/Bool/SetInt/SeqInt/RecAB/Str expressions over a fixed state, asserts `eval_expr` and `eval_compiled(compile_expr(...))` agree on every case (or both error). Wired into `.github/workflows/diff-tlc.yml` at `PROPTEST_CASES=256`. **3 real divergence classes uncovered** (parked as T2.1–T2.3 below; not patched per agent rules). 11 tests in the file (10 sanity + 1 proptest).

### Bugs uncovered by T2 (block 1.0.0; fix before next phase)

- [ ] **T2.1. SOUNDNESS: compiled-expr parser fails on nested `[r EXCEPT !.f = v]`.** Minimised: `[[r EXCEPT !.a = 0] EXCEPT !.b = 0]`. Interpreter returns `Ok(Record({"a": 0, "b": 0}))`; compiler returns `Err(missing closing ']' in expression: [r)`. Same family also hit as `[[r EXCEPT !.a = 0] EXCEPT !.a = 0]` (multiple seeds — high prevalence). The compiler's bracket-matching in `compile_expr` (probably `src/tla/compiled_expr.rs`) loses depth tracking when the LHS of an outer `[...]` is itself a `[... EXCEPT ...]`. Suspected location: `compile_expr` / its `[`-aware tokeniser in `src/tla/compiled_expr.rs`.
- [ ] **T2.2. SOUNDNESS: compiled-expr parser fails on EXCEPT inside a record-literal field value.** Minimised: `[a |-> 0, b |-> ([r EXCEPT !.a = 0]).a]`. Interpreter returns `Ok(Record({"a": 0, "b": 0}))`; compiler returns `Err(unexpected trailing tokens in expr: a |)`. The compiler tokeniser confuses the inner `[r EXCEPT !.a = 0]` for the field-list delimiter when it's nested inside a `b |->` value. Suspected location: same `[`-handling in `src/tla/compiled_expr.rs` as T2.1, possibly the same root cause.
- [ ] **T2.3. SOUNDNESS: interpreter type-errors on `<range> \subseteq <set>` (and `\intersect`) when the range LHS is paren-wrapped or appears inside a filter-set body.** Minimised: `(1..3 \subseteq {0, 0})` — interpreter `Err(expected Set, got Int(3))`, compiler `Ok(Bool(false))`. Also surfaces nested in filter-set bodies: `({__e \in 1..4 : ((1..3 \subseteq S)) \/ (__e > 0)} \intersect {0, 0})`. The interpreter is reading `1..3` as the integer `3` (likely `..` precedence handling in `eval_expr` against `\subseteq`/`\intersect`). Suspected location: `eval_expr` operator-precedence handling for `..` versus set operators, in `src/tla/eval.rs`.
