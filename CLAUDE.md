...
- T10.2 (v1.1.0 + v1.2.1/v1.2.2/v1.2.4/v1.2.6): Streaming SCC discovery. Phase 1 oracle + phase 2 stages 1–5 (PageAlignedColorMap, protocol variants, hot-loop DFS lift, in-band fairness, multi-node cluster routing) all shipped. The streaming-SCC memory win is realized — DFS exploration no longer materializes the labeled-transitions adjacency map. The `--dfs-cluster-listen` user-facing CLI flag is wired and validated end-to-end over real cross-machine TCP: a 4-node cluster-placement-group spot run on WorkQueue.tla partitions disjointly (per-node distinct sums exactly to the single-node baseline at N=2/3/4) and survives concurrent swarm failpoints. Cross-process correctness depends on `Model::fingerprint` being deterministic across processes — see `src/model.rs::fingerprint_hasher` (fixed-seed; NOT `ahash::AHasher::default()`, which is per-process random) and the guards in `fingerprint_determinism_tests` + `tests/dfs_cluster_real_tcp.rs`.
