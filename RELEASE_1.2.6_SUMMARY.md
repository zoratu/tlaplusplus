...
- T13.4 Phases 2+3 — shipping-code Verus rewrite of `FingerprintShard`. Substantial design-and-engineering verification work, but no longer blocked on any `vstd` capability gap. All three originally-documented gaps resolve in current Verus features:
  - **Gap 1** via `tokenized_state_machine!` + counting permissions. PoC at `verification/verus/atomic_ptr_with_epoch.rs` (16 verified, 0 errors). Exec-wired prototype at `verification/verus/shard_exec_wired.rs` (15 verified, 0 errors) — a `VerifiedShard` struct with the full lifecycle: `new` / `read` / `cas_insert` / `clone` (CAS loop on rc_cell + protocol `do_clone`) / `dispose` (refcount drain + `dec_to_zero` + `ptr.free`) plus a `demonstrate_swap_pattern` exec demo of two coexisting epochs.
  - **Gap 2** via the `external_body` template that `vstd::raw_ptr::allocate` itself uses. Verified wrapper at `verification/verus/mmap_external_body.rs` (1 verified, 0 errors) with `mmap_allocate_huge_pages` + `munmap_huge_pages` modeled exactly on `vstd::raw_ptr::allocate` / `deallocate`.
  - **Gap 3** via `vstd::logatom` logically-atomic specifications (or `Shared<AtomicInvariant>` per arc.rs, as used in `shard_exec_wired.rs`).
  See `T13.2-T13.4-design.md` for the cross-survey of upstream templates and the updated gap analysis.
