...
### PR #142 — general `[](P => <>[]Q)` graph-level liveness check

- **Parser** (`src/tla/temporal.rs`): a new `TemporalFormula::Implies` variant and top-level temporal-`=>` parsing (precedence below `/\`/`\/`, above `<=>`). A guardrail keeps a pure `state-pred => state-pred` as a single `StatePredicate`, so `[](A => B)` box-safety lowering (v1.2.21) and ACP_NB_WRONG's `AC1` are preserved. `[](P => <>[]Q)` now parses to `Always(Implies(StatePredicate(P), EventuallyAlways(StatePredicate(Q))))`.
- **Classification**: `Always(Implies(_, <>[]Q))` classifies as a liveness property so the post-processing pass runs.
- **Graph-level check** (`src/models/tla_native.rs`, `src/runtime/liveness.rs`): a violation exists iff a reachable `P`-state can reach a "bad cycle" where `<>[]Q` fails. A per-SCC check is insufficient — RealTime's trigger `P = (now # 4)` holds on the *stem*, while the bad `now = 4` cycle has `¬Q` — so the check works over the full reachable graph. With no fairness, stuttering (`[][Next]_vars`) makes any reachable `¬Q` state a bad cycle; with `WF_`/`SF_` fairness, the bad cycle must be a *fair* non-trivial SCC (a stutter-forever behaviour is excluded by fairness), reusing the existing fairness-SCC machinery.
- **Soundness guard (d)**: an independent temporal-logic review (Codex) confirmed the design and flagged that the no-fairness stutter path is only sound when the behaviour spec is exactly `Init /\ [][Next]_vars`; a `SPECIFICATION` conjoining a non-fairness temporal assumption (`[]<>R`, `<>R`, `~>`, …) excludes the stutter suffix. A new `spec_has_non_fairness_liveness` flag (computed via `formula_has_non_fairness_liveness` over the parsed `SPECIFICATION`) skips the no-fairness path when such an assumption is present — conservative (miss rather than false-flag). INIT/NEXT configs have no `SPECIFICATION` formula, so the flag is false and RealTime (which uses `INIT BigInit` / `NEXT BigNext`) still fires; `WF_`/`SF_` are extracted as fairness constraints and route to the fairness path.

`MCRealTimeHourClock` now reports `violation=true` at 216 distinct, matching TLC. A 226-spec corpus scan in both directions (from the pre-guard implementation, which the conservative guard is a strict subset of) showed +1 fixed (RealTime), zero new false violations, and zero newly-missed violations; an intermediate `nbacg_guer01` regression was caught and fixed during development. Gauntlet green: `cargo test --release` 0 failures (+5 parser regression tests), `scripts/diff_tlc.sh` 13/13 (the fairness+liveness `work_queue` gate stays green), compiled-vs-interpreted proptest 17/17, state-graph snapshots 12/12.

## v1.2.24 (2026-07-06)

Wave 2 of the corpus diff-vs-TLC audit: five soundness fixes across ~13 root causes, investigated in parallel by spec-family and validated together in one integration build. All named target specs match TLC's exact distinct-state count and verdict; a full 226-spec corpus scan in both directions found **zero new false-violations and zero newly-missed violations**. Gauntlet green: `scripts/diff_tlc.sh` 13/13, compiled-vs-interpreted proptest 17/17, state-graph snapshots 12/12, `cargo test --release` (modulo one pre-existing flaky checkpoint proptest, unrelated to these changes).

### PR #136 — gate SCC fairness violations on actual liveness-property failure

The fairness SCC pass (`check_fairness_on_scc_fp_sharded`) was property-blind: it reported *any* strongly-connected component lacking the fair action as a `Liveness` violation. A fairness-unfair cycle is a behaviour TLC *excludes*, not a counterexample. This surfaced as a regression on `ewd840_EWD840` (v1.2.20's #125 correctly improved exploration 192→302, exposing the false positive). A new `Model::scc_violates_liveness_property` checks whether a declared temporal property genuinely fails on a flagged SCC — `Some(true)` reports, `Some(false)` suppresses, `None` falls back to prior behaviour; `TlaModel` implements recurrent-cycle semantics for `<>P`, `[]<>P`, `P ~> Q`, `<>[]P`. Additive (the fairness pass is unchanged), wired into BFS and opt-in DFS. Clears `ewd840_EWD840` (302), `ewd840_SyncTerminationDetection` (129), `ewd998_AsyncTerminationDetection` (4097) — all no violation, matching TLC. Genuine liveness violations are still caught (verified, including the `work_queue` diff_tlc gate).

### PR #137 — quantifier body consumes `=>` and `<=>`, not just `/\` and `\/`

The interpreted `split_top_level` suppressed splitting inside a `\A`/`\E` quantifier body for `/\` and `\/` but not `=>`/`<=>`. A TLA+ quantifier body extends as far right as possible, so `X /\ \A y \in S : P => Q` (which must parse as `X /\ (\A y : (P => Q))`) had its `=>` lifted to top level and re-associated into `(X /\ \A y : P) => Q` — vacuously TRUE when the antecedent is false. On `SlidingPuzzles`, `ChooseOne(S, P(_)) == CHOOSE x \in S : P(x) /\ \A y : P(y) => y = x` mis-evaluated, so the reachable space collapsed to 2 states and **hid the `KlotskiGoal` violation** (ours reported SAFE where TLC finds it). Extending the in-quantifier-body suppression to `=>`/`<=>` makes ours explore the full space and flag the violation. General fix for any `\A … : P => Q` inside a conjunction.

### PR #138 — init false-violations: SUBSET-in-function-codomain membership + multi-binder map comprehension

`btree/kvstore`: a multi-binder map-comprehension parse bug caused a false violation → now 2641, no violation, matching TLC. `NanoBlockchain`: `x \in SUBSET S` inside a function codomain tried to materialize a huge powerset → a "too large" error reported as an initial-state violation; an interpreted-path structural membership short-circuit (complementing v1.2.23 #131's compiled-path fix) fixes the `TypeInvariant` false-violation. (NanoBlockchain still has a separate pre-existing `SafetyInvariant` record-access false-violation and a full-exploration perf limitation, both filed as follow-ups.)

### PR #139 — three under-exploration (false-safe) bugs

Ours generated far fewer successors than TLC on several specs — a false-safe risk. Three root causes: (1) a constant-operator override called with a *primed* argument (`Send(p,req,memInt,memInt')` with `Send <- MCSend`) is an action only via its primed arg, but `looks_like_action` inspected only body text → treated as a boolean guard → never fired (shared cause across CachingMemory + Liveness); (2) a nested `\E p : \E q : Recv(p,q) \/ Recv(q,p)` dropped the inner `\E q :` from the second branch → `q` unbound → no successors (lamport_mutex); (3) a `CASE`-of-actions evaluated as a boolean dropped the winning arm's primed assignment (ReadersWriters). Results = TLC exactly: MCInternalMemory 4408, MCWriteThroughCache 5196, MCLiveInternalMemory 4408, MCLiveWriteThroughCache 5196, MCLamportMutex 724274, ReadersWriters 21527. (Enabling MCLiveInternalMemory's exploration surfaces a separate pre-existing fairness false-positive — a confirmed false-positive, not a missed violation — filed as a follow-up.)

### PR #140 — false-violation + undercount: Init var deps, user infix ops, LET/RECURSIVE parsing

Specs that undercounted *and* false-violated. Four eval/parse causes: (1) an Init equality on a set-membership var (`ack = rdy` where `rdy \in {0,1}`) resolved the not-yet-bound RHS to a bogus `ModelValue("rdy")` in the eager-equality phase, corrupting state — now deferred to the late-equality phase (shared cause across the Specifying-Systems specs); (2) user-defined `\`-infix operators (`\preceq`, `\sqsubseteq`) mis-split as set-minus in both interpreted and compiled paths; (3) module def-body truncation on LET-local `==` / nested `RECURSIVE`; (4) the interpreted LET binding parser swallowing a trailing `RECURSIVE` line. Results = TLC: AsynchInterface 12, APAsynchInterface 8, ABCorrectness 20, Chameneos 34534, LeastCircularSubstring 8554 — all no violation. MCAlternatingBit's false violation is fixed (verdict correct); a count-only over-exploration gap (1476 vs 240) remains as a documented follow-up.
