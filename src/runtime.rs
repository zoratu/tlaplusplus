...
pub struct EngineConfig {
    ...
    /// Distributed work stealer for multi-node model checking.
    /// When `Some`, enables work stealing and bloom filter exchange across nodes.
    /// When `None` (default), single-node mode with zero overhead.
    pub distributed_stealer: Option<Arc<DistributedWorkStealer>>,
    /// Receiver for states stolen from remote nodes. Workers drain this when idle.
    pub stolen_states_rx: Option<crossbeam_channel::Receiver<StolenState>>,
    /// Sender for states to donate to remote nodes on steal requests.
    pub donate_states_tx: Option<crossbeam_channel::Sender<Vec<u8>>>,
    /// Receiver end of the donate channel — passed to the inbound handler.
...
use pause::{
    CheckpointPauseQueue, PauseController, pause_worker_after_empty_pop_during_checkpoint,
...
use checkpoint::{
    ...
    write_checkpoint_manifest, write_validated_rolling_checkpoint,
};
use memory::{
    apply_memory_budget, compute_effective_memory_max,
    should_enable_file_backed_fingerprint_store, should_start_fingerprint_memory_monitor,
};
use shards::calculate_optimal_shard_count;
    ...
    M: Model,
{
    ...
    let workers: Vec<std::thread::JoinHandle<()>> = if use_dfs_path {
        ...
        for (worker_id, worker_state) in worker_states.into_iter().enumerate() {
            ...
            trace_count: Arc::clone(&trace_state_count),
            max_trace_states,
            distributed_stealer: config.distributed_stealer.clone(),
            stolen_rx: config.stolen_states_rx.clone(),
...
mod tests {
    ...
    // Quiescence schedule tests moved to runtime/pause.rs::tests.
    // file_backed_fingerprint_store_uses_effective_memory_cap and
    // memory_monitor_requires_file_backed_fingerprint_store moved to
    // runtime/memory.rs::tests.
    // worker_pauses_when_checkpoint_is_requested... and the two
    // request_checkpoint_pause/pause_worker_after_empty_pop_is_noop tests
