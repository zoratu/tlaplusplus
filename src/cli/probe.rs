//! Probe machinery for `analyze-tla` plus the unit-test suite.
//!
//! Extracted from `src/main.rs` as part of the cli/ refactor.
//! Kept as a single file (vs. the multi-file split in the design doc)
//! because the `probe_*` family of functions has tight mutual recursion
//! across the proposed file boundaries — see
...
pub(crate) fn parse_zero_arg_state_operator_ref(text: &str) -> Option<(Option<String>, String)> {
...
pub(crate) fn resolve_zero_arg_state_definition<'a>(
    expr: &str,
    definitions: &'a BTreeMap<String, TlaDefinition>,
    instances: &'a BTreeMap<String, TlaModuleInstance>,
) -> Option<(
...
pub(crate) fn expand_state_predicate_clauses(
    body: &str,
...
pub(crate) fn append_expanded_state_predicate_clause(
    clause: &str,
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
...
pub(crate) fn apply_instance_substitutions_to_text(expr: &str, instance: &TlaModuleInstance) -> String {
...
pub(crate) fn seed_probe_state_from_type_invariants(
...
pub(crate) fn seed_probe_state_from_membership_body(
    probe_state: &mut TlaState,
    body: &str,
    module: &TlaModule,
) -> usize {
...
pub(crate) fn current_probe_value_satisfies_type_constraint(
    var: &str,
...
#[derive(Clone, Copy)]
pub(crate) enum TypeInvariantConstraintKind {
    Membership,
    Subset,
}

pub(crate) fn collect_type_invariant_constraints(
    clause: &str,
    module: &TlaModule,
...
pub(crate) fn representative_value_from_type_constraint(
    constraint: &(TypeInvariantConstraintKind, String),
    ctx: &EvalContext<'_>,
) -> Option<TlaValue> {
    match constraint.0 {
...
pub(crate) fn should_refine_probe_value(current: &TlaValue, replacement: &TlaValue) -> bool {
...
pub(crate) fn should_refine_satisfied_probe_value(
    current: &TlaValue,
...
pub(crate) fn current_matches_definition_backed_singleton_member(
    current: &TlaValue,
    constraint_expr: &str,
    ctx: &EvalContext<'_>,
) -> bool {
...
pub(crate) fn semantic_constraint_parts(expr: &str) -> Vec<String> {
...
pub(crate) fn union_constraint_parts(expr: &str) -> Vec<String> {
...
pub(crate) fn function_set_range_expr(expr: &str) -> Option<&str> {
    let trimmed = strip_probe_outer_parens(expr.trim());
...
pub(crate) fn function_range_is_uniformly(current: &TlaValue, expected: &TlaValue) -> bool {
    match current {
...
pub(crate) fn is_definition_backed_probe_expr(expr: &str, ctx: &EvalContext<'_>) -> bool {
...
pub(crate) fn should_refine_indeterminate_probe_value(current: &TlaValue, replacement: &TlaValue) -> bool {
...
pub(crate) fn should_replace_probe_value_after_type_mismatch(current: &TlaValue) -> bool {
    match current {
        TlaValue::Set(values) => values.is_empty(),
        TlaValue::Seq(values) => values.is_empty(),
        TlaValue::Record(fields) => fields.is_empty(),
...
pub(crate) fn probe_value_score(value: &TlaValue) -> u32 {
    match value {
...
pub(crate) fn representative_value_from_set_expr(set_expr: &str, ctx: &EvalContext<'_>) -> Option<TlaValue> {
...
pub(crate) fn representative_value_from_definition_body(
    body: &str,
    ctx: &EvalContext<'_>,
) -> Option<TlaValue> {
...
pub(crate) fn is_probe_simple_identifier(expr: &str) -> bool {
...
pub(crate) fn representative_member_from_domain_expr(expr: &str, ctx: &EvalContext<'_>) -> Option<TlaValue> {
...
pub(crate) fn try_create_representative_value_from_type_expr(
    set_expr: &str,
    ctx: &EvalContext<'_>,
) -> Option<TlaValue> {
    let trimmed = strip_probe_outer_parens(set_expr.trim());
...
pub(crate) fn representative_value_from_union_parts(expr: &str, ctx: &EvalContext<'_>) -> Option<TlaValue> {
    for delim in ["\\cup", "\\union"] {
...
pub(crate) fn representative_value_from_operator_call_expr(
    expr: &str,
    ctx: &EvalContext<'_>,
) -> Option<TlaValue> {
...
pub(crate) fn resolve_probe_operator_call_definition<'a>(
    name: &str,
    ctx: &'a EvalContext<'a>,
) -> Option<(
    &'a TlaDefinition,
...
pub(crate) fn try_create_representative_record(set_expr: &str, ctx: &EvalContext<'_>) -> Option<TlaValue> {
    let trimmed = strip_probe_outer_parens(set_expr.trim());
    let inner = trimmed.strip_prefix('[')?.strip_suffix(']')?;
...
#[cfg(test)]
pub(crate) fn sample_param_value(param: &str, probe_state: &TlaState) -> TlaValue {
    sample_param_value_with_context(param, probe_state, &BTreeMap::new(), &BTreeMap::new())
}

pub(crate) fn sample_param_value_with_context(
    param: &str,
...
pub(crate) fn infer_action_param_samples_from_next(
    next_body: &str,
    probe_state: &TlaState,
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
...
pub(crate) fn merge_action_param_samples(
...
pub(crate) fn infer_next_param_locals_from_spec(
    next_name: &str,
    next_params: &[String],
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
...
pub(crate) fn extract_exists_binders_for_next_call(
    text: &str,
    next_name: &str,
    next_params: &[String],
    probe_state: &TlaState,
...
pub(crate) fn infer_action_param_samples_from_module_contexts(
...
pub(crate) fn expr_probe_is_ready(expr_total: usize, expr_ok: usize) -> bool {
...
pub(crate) fn is_probe_sampling_limitation_error(err: &anyhow::Error) -> bool {
...
pub(crate) fn collect_action_param_samples_from_expr(
    expr: &str,
...
pub(crate) fn split_probe_quantifier(expr: &str) -> Option<(&str, &str)> {
    let colon_idx = find_probe_top_level_char(expr, ':')?;
    Some((expr[..colon_idx].trim(), expr[colon_idx + 1..].trim()))
}

pub(crate) fn sample_exists_quantifier_binders(
    binder_text: &str,
    locals: &BTreeMap<String, TlaValue>,
    probe_state: &TlaState,
    definitions: &BTreeMap<String, TlaDefinition>,
...
pub(crate) fn parse_probe_action_call(expr: &str) -> Option<(String, Vec<String>)> {
...
pub(crate) fn parse_probe_action_if(expr: &str) -> Option<(&str, &str, &str)> {
    let trimmed = expr.trim();
...
pub(crate) fn next_probe_word(input: &str, from: usize) -> Option<(&str, usize, usize)> {
    let mut i = from;
    while i < input.len() {
...
pub(crate) fn find_probe_outer_then(input: &str) -> Option<usize> {
...
pub(crate) fn find_probe_outer_else(input: &str) -> Option<usize> {
    let mut nested_if = 0usize;
    let mut i = 0usize;
    while let Some((word, start, end)) = next_probe_word(input, i) {
        match word {
...
pub(crate) fn parse_probe_identifier_prefix(expr: &str) -> Option<(String, &str)> {
...
pub(crate) fn take_probe_group(expr: &str, open: char, close: char) -> Option<(&str, &str)> {
...
pub(crate) fn strip_probe_outer_parens(expr: &str) -> &str {
...
pub(crate) fn strip_probe_comments(expr: &str) -> String {
    expr.lines()
...
pub(crate) fn normalize_probe_clause_expr(expr: &str) -> String {
...
pub(crate) fn find_probe_matching_paren(expr: &str) -> Option<usize> {
...
pub(crate) fn find_probe_top_level_char(expr: &str, target: char) -> Option<usize> {
    find_probe_top_level_char_from(expr, target, 0)
}

pub(crate) fn find_probe_top_level_char_from(expr: &str, target: char, start: usize) -> Option<usize> {
...
pub(crate) fn find_probe_top_level_keyword(expr: &str, keyword: &str) -> Option<usize> {
...
pub(crate) fn build_action_expr_probe_context<'a>(
...
pub(crate) fn seed_nonempty_sequence_probe_locals(
    probe_state: &TlaState,
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
    clauses: &[ActionClause],
...
pub(crate) fn find_sequence_accessed_vars(clauses: &[ActionClause]) -> BTreeSet<String> {
...
pub(crate) fn infer_sequence_element_probe_values(
    probe_state: &TlaState,
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
    clauses: &[ActionClause],
...
pub(crate) fn guess_probe_field_value(field: &str, ctx: &EvalContext<'_>) -> TlaValue {
...
pub(crate) fn collect_sequence_access_vars(expr: &str, out: &mut BTreeSet<String>) {
...
pub(crate) fn collect_sequence_accessed_record_fields(
    expr: &str,
...
pub(crate) fn collect_sequence_indexed_function_domains(
    expr: &str,
    out: &mut BTreeMap<String, BTreeSet<String>>,
) {
    for prefix in ["Head(", "Tail("] {
...
pub(crate) fn find_sequence_index_target(expr: &str, call_start: usize) -> Option<String> {
...
pub(crate) fn trim_probe_ascii_end(expr: &str, mut end: usize) -> usize {
...
pub(crate) fn find_except_target_name(prefix: &str) -> Option<String> {
...
pub(crate) fn infer_action_param_samples_from_guards(
    params: &[String],
    clauses: &[ActionClause],
    probe_state: &TlaState,
    definitions: &BTreeMap<String, TlaDefinition>,
...
pub(crate) fn boolean_param_sample_from_expr(
    expr: &str,
...
pub(crate) fn boolean_param_samples_from_nested_ifs(
    expr: &str,
    param_names: &BTreeSet<String>,
) -> Vec<(String, TlaValue)> {
    let chars: Vec<char> = expr.chars().collect();
...
pub(crate) fn matches_probe_keyword_at(chars: &[char], i: usize, keyword: &str) -> bool {
...
pub(crate) fn is_probe_word_char(ch: char) -> bool {
    ch.is_ascii_alphanumeric() || ch == '_'
}

pub(crate) fn refine_param_sample_from_clause_domains(
    param: &str,
    current: &TlaValue,
    clauses: &[ActionClause],
    probe_state: &TlaState,
...
pub(crate) fn find_clause_function_domains_for_params(expr: &str, param_names: &[&str]) -> BTreeSet<String> {
    let mut names = BTreeSet::new();
    let mut i = 0usize;
    while i < expr.len() {
...
pub(crate) fn find_indexed_function_name(prefix: &str) -> Option<String> {
    let trimmed = prefix.trim_end();
...
pub(crate) fn should_skip_action_expr_probe(expr: &str) -> bool {
    let trimmed = strip_probe_outer_parens(expr.trim());
    if trimmed.is_empty() {
        return true;
    }
...
pub(crate) fn starts_with_temporal_box(expr: &str) -> bool {
    expr.starts_with("[]")
}

pub(crate) fn starts_with_temporal_diamond(expr: &str) -> bool {
    expr.starts_with("<>") && !expr.starts_with("<<")
}

pub(crate) fn contains_temporal_box(expr: &str) -> bool {
    expr.contains("[]")
}

pub(crate) fn contains_temporal_diamond(expr: &str) -> bool {
...
pub(crate) fn merge_staged_prime_locals(from: &EvalContext<'_>, into: &mut EvalContext<'_>) {
...
pub(crate) fn should_short_circuit_probe_guard(expr: &str) -> bool {
...
pub(crate) fn probe_action_body_into_ctx(body: &str, ctx: &mut EvalContext<'_>) -> anyhow::Result<()> {
...
pub(crate) fn probe_action_body_via_runtime_eval(
    body: &str,
...
pub(crate) fn probe_disjunctive_action_body(
    expr: &str,
    ctx: &mut EvalContext<'_>,
) -> Option<anyhow::Result<()>> {
    let trimmed = strip_probe_outer_parens(expr.trim());
...
pub(crate) fn seed_probe_instance_constant_bindings(
...
pub(crate) fn bind_probe_instance_substitutions(
    instance: &TlaModuleInstance,
    parent_ctx: &EvalContext<'_>,
...
pub(crate) fn resolved_instance_primed_substitution_value(
    trimmed: &str,
    parent_ctx: &EvalContext<'_>,
) -> Option<TlaValue> {
...
pub(crate) fn body_contains_probeable_action_call(body: &str) -> bool {
    let normalized = strip_probe_comments(body);
...
pub(crate) fn definition_is_probeable_action(def: &TlaDefinition) -> bool {
    looks_like_action(def) || body_contains_probeable_action_call(&def.body)
}

pub(crate) fn body_contains_contextual_probeable_action_call(
    body: &str,
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
...
pub(crate) fn action_call_is_contextually_probeable(
    name: &str,
    arg_count: usize,
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
...
pub(crate) fn definition_is_contextually_probeable_action(
    def: &TlaDefinition,
    definitions: &BTreeMap<String, TlaDefinition>,
    instances: &BTreeMap<String, TlaModuleInstance>,
    visited: &mut BTreeSet<String>,
...
pub(crate) fn expand_probe_action_call(expr: &str, ctx: &mut EvalContext<'_>) -> Option<anyhow::Result<()>> {
    let trimmed = strip_probe_outer_parens(expr.trim());
...
pub(crate) fn current_probe_staged_primes(ctx: &EvalContext<'_>) -> BTreeMap<String, TlaValue> {
    ctx.locals
        .iter()
...
pub(crate) const PROBE_BRANCH_DISABLED_LOCAL: &str = "__probe_branch_disabled";

pub(crate) fn mark_probe_branch_disabled(ctx: &mut EvalContext<'_>) {
...
pub(crate) fn probe_branch_is_disabled(ctx: &EvalContext<'_>) -> bool {
    matches!(
...
pub(crate) fn probe_let_action_expr(expr: &str, ctx: &mut EvalContext<'_>) -> anyhow::Result<()> {
    let staged = current_probe_staged_primes(ctx);
...
pub(crate) fn probe_action_clause_expr(
    clause: &ActionClause,
    ctx: &mut EvalContext<'_>,
) -> Option<anyhow::Result<()>> {
...
pub(crate) fn parse_guard_exists_action_call(expr: &str) -> Option<(String, String)> {
    let trimmed = strip_probe_outer_parens(expr.trim());
...
pub(crate) fn probe_guard_exists_action_call(
    expr: &str,
    ctx: &mut EvalContext<'_>,
) -> Option<anyhow::Result<()>> {
...
pub(crate) fn probe_exists_action_body(
    binders: &str,
    body: &str,
    ctx: &mut EvalContext<'_>,
) -> anyhow::Result<()> {
...
pub(crate) fn probe_stuttering_action_expr(
    action: &str,
    vars: &[String],
    ctx: &mut EvalContext<'_>,
) -> anyhow::Result<()> {
...
pub(crate) fn pick_representative_from_set(set_val: &TlaValue) -> Option<TlaValue> {
    match set_val {
...
pub(crate) fn try_create_representative_function(set_expr: &str, ctx: &EvalContext<'_>) -> Option<TlaValue> {
    let trimmed = strip_probe_outer_parens(set_expr.trim());
    let inner = trimmed.strip_prefix('[')?.strip_suffix(']')?;
    let arrow_idx = find_probe_top_level_keyword(inner, "->")?;
